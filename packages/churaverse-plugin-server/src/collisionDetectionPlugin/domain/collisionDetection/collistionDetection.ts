...
/**
 * 衝突時に実行されるcallback. 衝突している2つのEntityを引数に取る
 */
export type OnOverlap<T1 extends ICollidableEntity, T2 extends ICollidableEntity> = (entity1: T1, entity2: T2) => void
 ...
 * @param onOverlap (entity1, entity2) => void
 */
export function detectEntityOverlap<T1 extends ICollidableEntity, T2 extends ICollidableEntity>(
  ...
  for (const entityId1 of Object.keys(collidedDict)) {
    ...
    for (const entityId2 of collidedDict[entityId1]) {
      ...
      if ((entity1 as ICollidableEntity) === (entity2 as ICollidableEntity)) continue
 ...
 */
function isCollided(rect1: IRectangle, rect2: IRectangle): boolean {
  const horizontal =
 ...
 */
function getCollidedDict<T1 extends ICollidableEntity, T2 extends ICollidableEntity>(
  entityRepository1: CollidableEntityRepository<T1>,
  entityRepository2: CollidableEntityRepository<T2>,
  currentIndex = 0,
  checkStack1: string[] = [],
 ...
 */
function hitTestInCell<T1 extends ICollidableEntity, T2 extends ICollidableEntity>(
